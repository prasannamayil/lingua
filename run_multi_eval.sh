...
cp "apps/main/configs/${CONFIG}" "${EVAL_DIR}/config.yaml"
...
find "${BASE_DIR}/checkpoints" -maxdepth 1 -type d -name "[0-9]*" | sort -n | while read -r ckpt_dir; do
    if [ -d "$ckpt_dir" ]; then
        ...
        sed "s|ckpt_dir:.*|ckpt_dir: ${ckpt_dir}|" "apps/main/configs/${CONFIG}" > "$TMP_CONFIG"
        ...
        torchrun --nproc_per_node=$NPROC -m apps.main.eval config="$TMP_CONFIG"
